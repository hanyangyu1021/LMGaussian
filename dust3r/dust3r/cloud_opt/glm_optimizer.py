...
class GLMOptimizer(BasePCOptimizer):
    ...
    def preset_depthmaps(self, known_depths, depth_msk=None):
        self._check_all_imgs_are_selected(depth_msk)

        if isinstance(known_depths, torch.Tensor) and known_depths.ndim == 2:
            known_depths = [known_depths]
        for idx, depth in zip(self._get_msk_indices(depth_msk), known_depths):
            if self.verbose:
                print(f' (setting pose #{idx} = {depth.shape})')
