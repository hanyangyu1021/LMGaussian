...
@torch.no_grad()
def init_minimum_spanning_tree_glm(self, **kw):
    """ Init all camera poses (image-wise and pairwise poses) given
        an initial set of pairwise estimations.
    """
    device = self.device
    pts3d, _, im_focals, im_poses = minimum_spanning_tree_glm(self.imshapes, self.edges,
                                                          self.pred_i, self.pred_j, self.conf_i, self.conf_j, self.im_conf, self.min_conf_thr,
                                                          device, has_im_poses=self.has_im_poses, verbose=self.verbose,
                                                          **kw)
...
def minimum_spanning_tree_glm(imshapes, edges, pred_i, pred_j, conf_i, conf_j, im_conf, min_conf_thr,
